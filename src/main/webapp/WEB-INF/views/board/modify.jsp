...
<section id="main-content">	
    ...
    <form name="writeform">
		...
		<input type="hidden" name="ntc_sq" value="${ntc_sq}">			
      ...
      <table class="table ns" ui-jq="footable" ui-options='{
        ...
        <tbody>       
          <tr>            
            <td>제목</td>
            <td><input name="ntc_nm" id="ntc_nm" type="text" style="width: 1050px;" value="${article.ntc_nm}"></td>            
          </tr>    
           <tr>            
            <td>내용</td>
            <td><textarea class="summernote" id="ntc_cnt" name="ntc_cnt">${article.ntc_cnt}</textarea></td>            
          </tr>            
        </tbody>
...
<script type="text/javascript">
  ...
  $("#registerBtn").click(function() {
  	if($("#ntc_nm").val() == "") {
  		...
  		$("form[name='writeform']").attr("method","post").attr("action", modifypath).submit();
  	}
  });
  
	$("#cencelBtn").click(function() {
		moveBoard('${ntc_div_sq}', '${ntc_div_nm}', '${ntc_sq}','${pg}', '${key}', '${word}', viewpath);  	
	});	
