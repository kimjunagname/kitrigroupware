...
public class CommonServiceImpl implements CommonService {
	...
	public PageNavigation getPageNavigation(Map<String, String> map) {
		int listSize = KitriConstance.BOARD_LIST_SIZE;//TODO 나중에 앨범게시판 만들때 수정 
		int pageSize = KitriConstance.NAVIGATOR_SIZE;
		int pg = Integer.parseInt(map.get("pg"));
		PageNavigation navigator = new PageNavigation();		
		navigator.setPageNo(pg);
		int newArticleCount = sqlSession.getMapper(CommonDao.class).getNewArticleCount(Integer.parseInt(map.get("ntc_div_sq")));
